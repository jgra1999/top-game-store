...
!(function(e) {
	...
	return (
		...
		(t.buildDots = function() {
			...
			if (e.opt.dots) {
				if (
					('string' == typeof e.opt.dots ? (e.dots = document.querySelector(e.opt.dots)) : (e.dots = e.opt.dots),
					e.dots)
				) {
				...
				[].forEach.call(d.slides, function(e, t) {
					...
					[].forEach.call(i, function(e) {
						...
						c === t || (h && h === t)
							? i.add('center')
							: (i.remove('center'),
							  i.add([t < c ? 'left' : 'right', Math.abs(t - (t < c ? c : h || c))].join('-')));
					...
					(d.scrollLock = setTimeout(function() {
						clearTimeout(d.scrollLock),
							0.02 < Math.abs(d.ele.scrollLeft / d.itemWidth - d.slide) &&
								(d.mouseDown ||
									(d.trackWidth > d.containerWidth + d.ele.scrollLeft && d.scrollItem(d.getCurrentSlide())));
		...
		(t.scrollItem = function(e, t, i) {
			...
			return (
				o.scrollTo(e, o.opt.duration * Math.abs(o.ele.scrollLeft - e), function() {
					o.updateControls(),
						o.emit('animated', { value: r, type: 'string' == typeof r ? 'arrow' : t ? 'dot' : 'slide' });
				}),
				!1
			);
		}),
